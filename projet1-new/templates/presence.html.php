...
// Vérifier si 'idpromo', 'etat', 'referentiel' et 'date' sont définis dans l'URL
if (isset($_GET['idpromo'], $_GET['etat'], $_GET['date'], $_GET['referentiel'])) {
    // Utiliser les valeurs de 'idpromo', 'etat', 'referentiel' et 'date' depuis l'URL
    $idpromo = $_GET['idpromo'];
    $etat = $_GET['etat'];
    ...
    // Vérifier si les valeurs de 'etat' et 'referentiel' sont égales à 'statut' et '' respectivement
    if ($etat == '' && $referentiel == '') {
        // Charger toutes les présences pour cette promotion et cette date
        //dd(listePresence());
        $presences_promo = chargerPresenceFiltres('presences.csv', $_SESSION['idPromoActive'], '', $date, '');
    } else {
        // Charger les présences filtrées pour cette promotion, cet état, cette date et ce référentiel
        $presences_promo = chargerPresenceFiltres('presences.csv', $idpromo, $etat, $date, $referentiel);
        // Stocker les données de présence dans la session
        $_SESSION['presence'] = $presences_promo;
        ...
        <div id="container-presence">
                ...
                <div class="recherhe">
                    <form action="" method="get">
                        ...
                        <select name="etat" id="etat">
                            <option value="" class="value">Statut</option>
                        ...
                        <select name="referentiel" id="reference">
                            ...
                            <?php foreach ($referentiels_promo as $referentiel) : ?>
                                <option value="<?php echo $referentiel['libelle']; ?>" class="value" <?php echo ($_GET['referentiel'] ?? '') == $referentiel['libelle'] ? 'selected' : ''; ?>><?php echo $referentiel['libelle']; ?></option>
                            <?php endforeach; ?>
                        </select>
            ...
            <div class="container-table">
                <table class="line5">
                    ...
                    <tbody>
                        <?php foreach ($_SESSION['presence'] as $presence) : ?>
